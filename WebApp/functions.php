...
class design
{
    ...
    {
        $StatusArrayDesc = ['Keine Auswahl', 'Überprüft', 'Ungültig', 'Beobachten', 'Duplizieren'];
        $StatusArrayDB = ['', 'R', 'V', 'F', 'D'];

        echo "<div class='form-group'>";
        echo "<label for='Status'>Status</label>";
        echo "<select id ='Status' name='Status' class='form-control'>";
        for ($i = 0; $i < count($StatusArrayDesc); $i++) {
    ...
    {
        $TypeArrayDesc = ['Zahlung', 'Gutschrift', 'Übertrag'];
        ...
        for ($i = 0; $i < count($TypeArrayDesc); $i++) {
    ...
    public static function input_account($TrAccountDefault)
    {
        ...
        for ($i = 0; $i < count($AccountArrayDesc); $i++) {
    ...
    public static function input_toaccount($TrToAccountDefault)
    {
        ...
        for ($i = 0; $i < count($ToAccountArrayDesc); $i++) {
...
class db_function
{
    ...
    public static function bankaccount_insert_json($BankAccountJSON)
    {
        ...
        $db->beginTransaction();
        $BankAccountListSize = count($BankAccountList);
   ...
   // Insert all categories
    public static function category_insert_json($CategoryJSON)
    {
        ...
        $db->beginTransaction();
        $CategoryListSize = count($CategoryList);
        ...
        while (db_function::db_version() !== $app_version) {
            switch (db_function::db_version()) {
                            case '0.9.2':
                                ...
                                break;
                            case '0.9.3':
                                self::upgrade_version('0.9.4');
                                break;
                            case '0.9.4':
                                self::upgrade_version('0.9.5');
                                break;
                            case '0.9.5':
                                self::upgrade_version('0.9.6');
                                break;
                            case '0.9.6':
                                self::to_0_9_7();
                                break;
                            case '0.9.7':
                                self::upgrade_version('0.9.8');
                                break;
                            case '0.9.8':
                                self::to_0_9_9();
                                break;
                            case '0.9.9':
                                self::upgrade_version('1.0.0');
                                break;
                            case '1.0.0':
